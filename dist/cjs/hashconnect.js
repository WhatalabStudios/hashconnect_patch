...
class HashConnect {
    ...
    async sendTransaction(topic, transaction) {
        transaction.byteArray = buffer_1.Buffer.from(transaction.byteArray).toString("base64");
    ...
    async sendTransactionResponse(topic, message) {
        if (message.receipt)
            message.receipt = buffer_1.Buffer.from(message.receipt).toString("base64");
        if (message.signedTransaction)
            message.signedTransaction = buffer_1.Buffer.from(message.signedTransaction).toString("base64");
    ...
    async authenticate(topic, account_id, server_signing_account, serverSignature, payload) {
        ...
        message.serverSignature = buffer_1.Buffer.from(message.serverSignature).toString("base64");
    ...
    async sendAuthenticationResponse(topic, message) {
        if (message.userSignature)
            message.userSignature = buffer_1.Buffer.from(message.userSignature).toString("base64");
        if (message.signedPayload)
            message.signedPayload.serverSignature = buffer_1.Buffer.from(message.signedPayload.serverSignature).toString("base64");
    ...
    generatePairingString(topic, network, multiAccount) {
        ...
        let pairingString = buffer_1.Buffer.from(JSON.stringify(data)).toString("base64");
        this.hcData.pairingString = pairingString;
        return pairingString;
    }
    decodePairingString(pairingString) {
        let json_string = buffer_1.Buffer.from(pairingString, 'base64').toString();
